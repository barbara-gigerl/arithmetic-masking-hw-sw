...
if [ $argv[1] = "parse" ]
  ...
  python3 -u $ALMA_HOME/verify.py --safe-graph $BUILD_DIR/safe_graph --circuit-graph $BUILD_DIR/circuit_graph --label $BUILD_DIR/labels_new.txt --vcd $BUILD_DIR/circuit.vcd --cycles 30 --order $order --mode transient --masking-type boolean  --dbg-output-dir $BUILD_DIR --rst-name rst_i --rst-cycles 6 --rst-phase 1  --debug-config default.conf > $BUILD_DIR/log.txt
